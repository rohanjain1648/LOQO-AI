...
## 🔄 Production-Ready Retry System Architecture (v2)

The retry system uses **programmatic validation gates**, **progressive prompt escalation**, **per-agent retry budgets (5 each)**, **parallel multi-agent retry**, and **best-of-N selection** to ensure maximum output quality:

```mermaid
graph TD
    START([START]) --> Extract["📥 extract_article"]
    Extract --> Editor["✍️ generate_narration"]
    Editor --> PreVal{"🔍 Pre-Validate<br/>Programmatic Gate"}
    PreVal -->|"Issues Found<br/>(duration, segments)"| Editor
    PreVal -->|"Clean"| Fork["⚡ fan_out_parallel"]
    Fork --> Visual["🎬 plan_visuals"]
    Fork --> Headline["📰 generate_headlines"]
    Visual --> PostVal{"🔍 Post-Validate<br/>Programmatic Gate"}
    Headline --> PostVal
    PostVal -->|"Auto-fixable<br/>(truncate, fade_out)"| PostVal
    PostVal -->|"Clean"| QA["🏆 review_quality<br/>LLM QA Scoring"]
    PostVal -->|"Visual fails"| Visual
    PostVal -->|"Headline fails"| Headline
    PostVal -->|"Both fail"| ParallelRetry["🔄 retry_parallel_gate"]
    ParallelRetry --> Visual
    ParallelRetry --> Headline
    QA --> Router{"🔀 Smart Router<br/>Budget-Aware"}
    Router -->|"✅ Pass"| BestN["🎯 select_best<br/>Best-of-N"]
    Router -->|"Editor fails<br/>(cascades)"| Editor
    Router -->|"Visual fails"| Visual
    Router -->|"Headline fails"| Headline
    Router -->|"Both fail"| ParallelRetry
    Router -->|"Budget exhausted"| BestN
    BestN --> Format["📄 format_output"]
    Format --> END([END])

    style PreVal fill:#1e293b,stroke:#f59e0b,color:#f59e0b
    style PostVal fill:#1e293b,stroke:#f59e0b,color:#f59e0b
    style BestN fill:#1e293b,stroke:#10b981,color:#10b981
    style Router fill:#1e293b,stroke:#667eea,color:#667eea
```
...
| Feature | Description |
|---|---|
| **Pre-Validation Gate** | Catches duration/segment count issues *before* wasting Visual + Headline LLM calls |
| **Post-Validation Gate** | Auto-fixes trivial issues (headline truncation, fade_out enforcement) |
| **Progressive Escalation** | Retry prompts get stricter: Tier 1 (gentle) → Tier 2 (diff context) → Tier 3 (surgical fix list) |
| **Temperature Reduction** | 0.7 → 0.5 → 0.4 → 0.3 → 0.2 → 0.1 across retries |
| **Per-Agent Budgets** | Editor: 5, Visual: 5, Headline: 5 (independent, not shared) |
| **Parallel Retry** | Visual + Headline retry simultaneously when both fail |
| **Best-of-N Selection** | Composite scoring picks best attempt, not always the last |
| **Programmatic QA Fallback** | If LLM QA crashes: retry once → programmatic scoring → never blind force-pass |
| **Global Safety Cap** | Max 8 total QA cycles prevents infinite loops |
...
loqo ai/
│
├── app.py                              # 🖥️ Streamlit web interface (+ Retry Decisions tab)
├── main.py                             # 🖥️ CLI entry point (+ retry telemetry output)
...
├── src/
│   ├── __init__.py
│   ├── config.py                       # ⚙️ Gemini LLM + LangFuse + retry temperature schedule
│   ├── state.py                        # 📋 BroadcastState (shared state + retry history)
│   ├── graph.py                        # 🔗 LangGraph v2: validation gates + parallel retry
│   │
│   ├── agents/                         # 🤖 Agent implementations
│   │   ├── __init__.py
│   │   ├── extraction.py              # Agent 1: Article Extraction (no LLM)
│   │   ├── editor.py                  # Agent 2: News Editor (progressive escalation)
│   │   ├── visual.py                  # Agent 3a: Visual Packaging (progressive escalation)
│   │   ├── headline.py               # Agent 3b: Headline Generator (progressive escalation)
│   │   ├── qa.py                      # Agent 4: QA (per-agent budgets + smart routing)
│   │   ├── validators.py             # 🔍 Programmatic pre/post validation + auto-fix
│   │   └── selector.py               # 🎯 Best-of-N attempt selection
│   │
...
│   │
│   ├── prompts/                        # 💬 System & user prompts (3-tier escalation)
│   │   ├── __init__.py
│   │   ├── editor_prompts.py          # Editor persona + 3-tier retry templates
│   │   ├── visual_prompts.py          # Visual director + 3-tier retry templates
│   │   ├── headline_prompts.py        # Graphics producer + 3-tier retry templates
...
│       ├── __init__.py
│       ├── formatter.py               # Merges outputs → JSON + screenplay + telemetry
│       └── templates.py               # Text templates + retry telemetry template
```
...
| **Retry Tracking** | Shows which retry round is active + per-agent budgets |
| **Screenplay Tab** | Rich visual display of each segment with tags, headlines, narration |
| **JSON Tab** | Raw structured JSON output with quality metadata |
| **🆕 Retry Decisions Tab** | Agent budget bars, score progression chart, decision timeline |
